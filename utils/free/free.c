...
void mp_init(size_type max_pool_length, void (*error_callback)()) {
    ...
    size_type left = max_pool_length;

    mp.head = malloc(mp.bucket_number * sizeof(void *));
    mp.length = malloc(mp.bucket_number * sizeof(size_t *));
    mp.max_length = malloc(mp.bucket_number * sizeof(size_t *));
    ...
    for (int i = 0; i < mp.bucket_number; ++i) {
        ...
        if (mp.head[i] == NULL) {
            printf("init error. need " SIZE_TYPE_FORMAT " bytes\n",
                   max_pool_length);
            error_callback();
        }
    }
...
void mp_info() {
    for (int i = 0; i < mp.bucket_number; ++i) {
        printf("bucket %d, usage %zu/%zu %.2f, %zu left\n", i, mp.length[i],
               mp.max_length[i], (double)mp.length[i] / mp.max_length[i] * 100,
               mp.max_length[i] - mp.length[i]);
    }
    printf("total: max_length: " SIZE_TYPE_FORMAT
           "  now_length: " SIZE_TYPE_FORMAT "\n",
           mp_get_max_length(), mp_get_length());
}
size_type mp_get_length() {
    size_type now_length = 0;
    for (int i = 0; i < mp.bucket_number; ++i) {
...
void *mp_new(size_t length) {
    ...
    printf("Can not make more memory. The memory pool is only " SIZE_TYPE_FORMAT
           " bytes, "
           "and " SIZE_TYPE_FORMAT " used, can not make more %zu bytes\n",
